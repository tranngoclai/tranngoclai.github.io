...
window.createReadPathWorld = function(run) { return function(raw) {
  ...
  w.node('client', {label: 'Trình duyệt khách\n' + run.productId, pos: [X.client, Y.ground, Z.spine], size: S.actor, tone: 'subject', order: 0, hover: 'Nơi request GET /products/' + run.productId.replace('PID-', '') + ' bắt đầu', shape: 'client'});

  w.node('dns-resolver', {label: 'DNS Resolver\nTTL ' + LV.fmtTtl(LV.byKey('dns').ttlSec), pos: [X.browser, Y.ground, Z.resolve], size: S.engine, tone: 'system', order: 1, hover: 'Phân giải tên miền trước khi request tồn tại — không nằm trên đường đi dữ liệu', shape: 'hex'});

  w.node('browser-cache', {label: 'Browser Cache\nbundle immutable · giá no-store', pos: [X.browser, Y.ground, Z.spine], size: S.gate, tone: 'gate', order: 1, hover: 'Cache-Control/ETag do response header quyết định', shape: 'hex'});
  w.node('cdn-edge', {label: 'CDN Edge\nTTL ' + LV.fmtTtl(cdnLv.ttlSec), pos: [X.cdn, Y.ground, Z.spine], size: S.gate, tone: 'gate', order: 2, hover: 'Edge config + purge API', shape: 'hex'});
  w.node('lb-proxy', {label: 'LB / Reverse Proxy\nTTL ' + LV.fmtTtl(lbLv.ttlSec), pos: [X.lb, Y.ground, Z.spine], size: S.gate, tone: 'gate', order: 2, hover: 'Proxy config, cache tag', shape: 'hex'});
  ...
  w.node('redis', {label: 'Redis\nproduct:' + run.productId.replace('PID-', '') + ' · TTL ' + LV.fmtTtl(redisLv.ttlSec), pos: [X.app, Y.ground, Z.store], size: S.gate, tone: 'gate', order: 3, hover: 'App cache — key/value do code app quản lý', shape: 'redis'});

  w.node('orm-cache', {label: 'ORM Cache\nTTL ' + LV.fmtTtl(ormLv.ttlSec), pos: [X.orm, Y.ground, Z.spine], size: S.gate, tone: 'gate', order: 4, hover: 'Auto-invalidate qua signal save()/delete() của ORM', shape: 'hex'});
  w.node('db-engine', {label: 'DB Engine\nbuffer pool', pos: [X.db, Y.ground, Z.spine], size: S.engine, tone: 'engine', order: 4, hover: 'DB tự quản buffer pool theo WAL — không cần app can thiệp', shape: 'cylinder'});
  w.node('os-cache', {label: 'OS Page Cache\nkernel', pos: [X.os, Y.ground, Z.spine], size: S.engine, tone: 'engine', order: 4, hover: 'Kernel tự evict khi memory pressure', shape: 'grid'});
  w.node('disk', {label: 'Disk\nnguồn thật', pos: [X.disk, Y.ground, Z.spine], size: S.store, tone: 'store', order: 5, hover: 'Nơi mọi cache phía trước cuối cùng đọc lại nếu miss hết', shape: 'cylinder'});
