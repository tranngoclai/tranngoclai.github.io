...
window.createColdCacheWorld = function(run) { return function(raw) {
  ...
  w.node('deploy-event', {label: 'Deploy / Restart\nvừa xong', pos: [X.app, Y.ground, Z.resolve], size: S.actor, tone: 'system', order: 0, hover: 'Trigger duy nhất của toàn bộ scenario'});

  w.node('client', {label: 'Trình duyệt khách\n' + run.productId, pos: [X.client, Y.ground, Z.spine], size: S.actor, tone: 'subject', order: 0, hover: 'Request đầu tiên sau deploy/restart'});

  w.node('dns-resolver', {label: 'DNS Resolver\nvẫn ấm', pos: [X.browser, Y.ground, Z.resolve], size: S.engine, tone: 'system', order: 1, hover: 'TTL DNS không liên quan gì tới deploy/restart'});

  w.node('browser-cache', {label: 'Browser Cache\n(empty)', pos: [X.browser, Y.ground, Z.spine], size: S.gate, tone: 'danger', order: 1, hover: 'Bundle hash mới — key cũ không còn khớp'});
  w.node('cdn-edge', {label: 'CDN Edge\n(empty)', pos: [X.cdn, Y.ground, Z.spine], size: S.gate, tone: 'danger', order: 2, hover: 'URL asset đổi theo bundle hash mới'});
  w.node('lb-proxy', {label: 'LB / Reverse Proxy\n(empty)', pos: [X.lb, Y.ground, Z.spine], size: S.gate, tone: 'danger', order: 2, hover: 'Cache tag gắn với version build cũ'});
  ...
  w.node('redis', {label: 'Redis\n(empty)', pos: [X.app, Y.ground, Z.store], size: S.gate, tone: 'danger', order: 3, hover: 'RAM bị xoá sạch khi container restart'});

  w.node('orm-cache', {label: 'ORM Cache\n(empty)', pos: [X.orm, Y.ground, Z.spine], size: S.gate, tone: 'danger', order: 4, hover: 'Process mới = in-memory state mới tinh'});
  w.node('db-engine', {label: 'DB Engine\nbuffer pool empty', pos: [X.db, Y.ground, Z.spine], size: S.engine, tone: 'danger', order: 4, hover: 'DB vừa restart — buffer pool trống'});
  w.node('os-cache', {label: 'OS Page Cache\n(empty)', pos: [X.os, Y.ground, Z.spine], size: S.engine, tone: 'danger', order: 4, hover: 'Kernel restart — page cache trống'});
  w.node('disk', {label: 'Disk\nnguồn thật — luôn ấm', pos: [X.disk, Y.ground, Z.spine], size: S.store, tone: 'store', order: 5, hover: 'store duy nhất còn ấm ngay từ đầu'});
