...
window.createNodeWideOomWorld = function(R) { return function(raw) {
  ...
  w.node('allocator', {label: 'pending allocation\napp main +' + M.fmtMi(R.event.allocationMi) + ' · reclaim failed', pos: [KERNEL_COL[0], L.on(S.kernel[1]), KERNEL_Z], size: S.kernel, tone: 'engine', order: 2, hover: 'The allocation event and failed-reclaim premise exist before the action; aggregate Mi alone does not decide global OOM', shape: 'grid'});
