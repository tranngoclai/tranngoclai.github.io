...
window.createNodeWideOomWorld = function(R) { return function(raw) {
  ...
  w.node('allocator', {label: 'pending allocation\napp main +' + M.fmtMi(R.event.allocationMi) + ' · reclaim failed', pos: [KERNEL_COL[0], L.on(S.kernel[1]), KERNEL_Z], size: S.kernel, tone: 'engine', order: 2, hover: 'The allocation event and failed-reclaim premise exist before the action; aggregate Mi alone does not decide global OOM', shape: 'box'});
  w.node('oom', {label: 'global OOM\nkiller', pos: [KERNEL_COL[1], L.on(S.kernel[1]), KERNEL_Z], size: S.kernel, tone: 'engine', order: 2, hover: 'Ranks killable Linux tasks in the allowed allocation domain', shape: 'hex'});
  w.node('reaper', {label: 'OOM reaper', pos: [KERNEL_COL[2], L.on(S.kernel[1]), KERNEL_Z], size: S.kernel, tone: 'engine', order: 2, hover: 'Reclaims the victim address space', shape: 'hex'});
  ...
  w.node('best-effort', {label: be.qosClass + ' · restartPolicy ' + be.restartPolicy + '\ncpu/mem requests + limits: none', pos: [POD_COL[0], L.on(S.pod[1]), POD_Z], size: S.pod, tone: 'peer', order: 4, hover: 'All four CPU/memory request/limit fields are explicitly none; derived oom_score_adj 1000', shape: 'seal'});
  w.node('burstable', {label: bu.qosClass + ' · restartPolicy ' + bu.restartPolicy + '\nmem r' + M.fmtMi(buResources.requests.memoryMi) + '/l' + M.fmtMi(buResources.limits.memoryMi) + ' · cpu none', pos: [POD_COL[1], L.on(S.pod[1]), POD_Z], size: S.pod, tone: 'subject', order: 4, hover: 'Complete initial resource spec; QoS and adjustment are derived rather than authored', shape: 'seal'});
  w.node('guaranteed', {label: gu.qosClass + ' · restartPolicy ' + gu.restartPolicy + '\ncpu r' + M.fmtCpu(guResources.requests.cpuMilli) + '/l' + M.fmtCpu(guResources.limits.cpuMilli) + ' · mem r' + M.fmtMi(guResources.requests.memoryMi) + '/l' + M.fmtMi(guResources.limits.memoryMi), pos: [POD_COL[2], L.on(S.pod[1]), POD_Z], size: S.pod, tone: 'live', order: 4, hover: 'Every container has equal, explicit CPU and memory requests/limits; derived oom_score_adj -997', shape: 'seal'});

  w.node('best-effort-process', {label: 'worker PID 1 · main\n' + M.fmtMi(byKey['best-effort-process'].usageMi) + ' · adj ' + byKey['best-effort-process'].oomScoreAdj, pos: [POD_COL[0], L.onTopOf(S.pod[1], S.proc[1]), POD_Z], size: S.proc, tone: 'peer', order: 5, hover: 'Killable container-init process; adjustment is finite, not infinite', shape: 'client'});
  w.node('burstable-process', {label: 'app PID 1 · main\n' + M.fmtMi(byKey['burstable-process'].usageMi) + ' · adj ' + byKey['burstable-process'].oomScoreAdj + ' · next +' + M.fmtMi(R.event.allocationMi), pos: [POD_COL[1], L.onTopOf(S.pod[1], S.proc[1]), POD_Z], size: S.proc, tone: 'subject', order: 5, hover: 'This predeclared process owns the failed node allocation event and has the highest finite teaching projection', shape: 'client'});
  w.node('guaranteed-process', {label: 'api PID 1 · main\n' + M.fmtMi(byKey['guaranteed-process'].usageMi) + ' · adj ' + byKey['guaranteed-process'].oomScoreAdj, pos: [POD_COL[2], L.onTopOf(S.pod[1], S.proc[1]), POD_Z], size: S.proc, tone: 'live', order: 5, hover: 'Strongly protected, but not absolutely immune', shape: 'client'});
