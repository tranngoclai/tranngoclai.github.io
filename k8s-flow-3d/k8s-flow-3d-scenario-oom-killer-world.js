...
window.createOomKillerWorld = function(R) { return function(raw) {
  ...
  w.node('node', {label: 'Worker A\n' + M.fmtMi(R.node.memoryCapacityMi) + ' capacity · cgroup v2', pos: [L.X.node, L.Y.slab, A], size: S.nodeDeep, tone: 'surface', order: 1, hover: 'Linux host running this Pod; the cgroup version is an initial resource fact', shape: 'slab'});
  ...
  w.node('pod', {label: pod.qosClass + ' · restartPolicy ' + pod.restartPolicy + '\nmem r' + M.fmtMi(requestMi) + ' / l' + M.fmtMi(limitMi), pos: [POD_COL, L.on(S.pod[1]), POD_Z], size: S.pod, tone: 'subject', order: 4, hover: 'Initial Pod resource spec: CPU request/limit are both explicitly none; memory request=' + M.fmtMi(requestMi) + ', limit=' + M.fmtMi(limitMi) + ', restartPolicy=' + pod.restartPolicy, shape: 'seal'});
  w.node('process', {label: 'PID 1 · main · Running\n' + M.fmtMi(R.cgroup.memoryCurrentMi) + ' → next +' + M.fmtMi(R.event.allocationMi), pos: [POD_COL, L.onTopOf(S.pod[1], S.proc[1]), POD_Z], size: S.proc, tone: 'subject', order: 5, hover: R.process.name + ' is the killable container-init process; the authored allocation event already belongs to it', shape: 'client'});
